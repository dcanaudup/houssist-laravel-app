    ...
    <div class="py-4 space-y-4">
        ...
        <div class="flex justify-between">
            <div class="w-2/4 flex space-x-4">
                <x-input.text wire:model.debounce.300ms="filters.search" placeholder="Search Username..." type="search"/>
            ...
            @if ($showFilters)
                <div class="bg-cool-gray-200 p-4 rounded shadow-inner flex relative">
                    ...
                    <div class="w-1/2 pl-2 space-y-4">
                        <x-input.select wire:model.lazy="filters.status" id="filters.withdrawal_type" placeholder="Select an option...">
                            @foreach (\Spatie\LaravelOptions\Options::forEnum(\App\Modules\Shared\Enums\WithdrawalStatus::class)->toArray() as $option)
                                <option value="{{ $option['value'] }}">{{ $option['label'] }}</option>
                            @endforeach
                        </x-input.select>
                ...
                <x-slot:body>
                    @forelse($withdrawals as $withdrawal)
                        <x-table.row wire:loading.class.delay="opacity-50" class="bg-cool-gray-200" wire:key="row-{{$withdrawal->withdrawal_id}}">
                            <x-table.cell>
                                <span class="text-cool-gray-900 font-medium">{{ $withdrawal->user->username }} </span>
                            </x-table.cell>
                            <x-table.cell>
                                {{ $withdrawal->amount }}
                            </x-table.cell>
    ...
    <x-modal.dialog wire:model.defer="showViewModal" max-width="4xl">
        ...
        <x-slot name="content">
            <x-label.group label="User">
                <dd class="mt-1 text-sm text-gray-900">{{ $viewWithdrawal?->user->username }}</dd>
            </x-label.group>
        ...
        <x-slot name="footer">
            ...
            @if($viewWithdrawal?->status == \App\Modules\Shared\Enums\WithdrawalStatus::Pending)
                <x-button.danger onclick="confirm('Are you sure you want to reject this withdrawal?') || event.stopImmediatePropagation()" wire:click="reject({{$viewWithdrawal?->withdrawal_id}})">Reject</x-button.danger>
                <x-button.primary onclick="confirm('Are you sure you want to approve this withdrawal?') || event.stopImmediatePropagation()" wire:click="approve({{$viewWithdrawal?->withdrawal_id}})">Approve</x-button.primary>
            @endif
        </x-slot>
    </x-modal.dialog>
</div>
